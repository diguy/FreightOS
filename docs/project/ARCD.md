...
- 客户可见最终回答层的真实异常回归和生产级观测。内部工具结果继续保持结构化，
  不能直接展示给用户。
- 最近会话持久化与用户身份绑定，Redis 保存短期会话状态，MySQL 保存必要的会话索引或摘要。
- 第一阶段只实现电脑网页和消费者 API，不提前引入物流公司后台和多租户管理。

当前 Dify 已使用双阶段 Chatflow：理解阶段输入当前消息、经过裁剪的历史摘要、
最近对话、当前槽位和系统提示词；Python 负责最终状态合并、权限校验和工具门控；
回答阶段接收有效会话上下文和结构化工具结果，并只输出客户可见的 `final_answer`。
...
- 当前已使用 Dify v6 双阶段回答契约，回答阶段输出 `final_answer`。
- Python 会校验 `final_answer`，并在回答阶段失败或输出异常时保留确定性安全兜底。
- 已完成“存在待处理任务”和“已完成且无待处理任务”的状态隔离；未知或模糊消息
  在存在待处理任务时仍按待处理任务提示缺失信息或要求确认。
